...
class BlogView(APIView):
    ...
    def get(self, request):
        ...
        if search_query:
            blogs_objects = Blog.objects.filter(
                Q(title__icontains=search_query)
                | Q(user__username=search_query)
                | Q(blog_text__contains=search_query)
            )
        else:
        ...
        serializer = BlogSerializer(
            blog_paginator.page(page_number).object_list, many=True
        )
        all_data = {
...
def user_blogs(request):
    if request.method == "GET":
        ...
        if token:
            ...
            page_number = valid_page_num_for_pagination(
                request, blog_paginator.num_pages
            )
